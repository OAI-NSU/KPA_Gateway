...
class FrameMessage(AbstractFrame):
    ...
    @staticmethod
    def parse(data: bytes) -> 'FrameMessage':
        ...
        return FrameMessage(*strings)

    def __str__(self) -> str:
        return f'ID: {self.frame_id}\nAmount: {self.str_amount}\n' + '\n'.join(self.strings)


class FrameLogMessage(AbstractFrame):
    ...
    @staticmethod
    def parse(data: bytes) -> 'FrameLogMessage':
        message_type: int = struct.unpack('<H', data[:2])[0]
        message: str = data[2:].decode('utf-8')
        return FrameLogMessage(message_type, message)
